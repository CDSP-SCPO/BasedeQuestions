	...
	)
	{
		...
		while ($i < $l - 1)
		{
			...
			if (abs($pos1 - $pos2) <= $acceptedCommonStartLengthDifference)
			{
				...
				$start1 = trim($start1); // Most errors came from an leading or trailing espace in a short start
				$start2 = substr($variables[$i + 1]['question']['question_text'], 0, $pos2);
				$start2 = trim($start2); // Most errors came from an leading or trailing espace in a short start
				similar_text($start1, $start2, $percent);
				
				if (! ($percent >= $commonStartMinSim))
				{			
					$i++;
					continue;
				}
			...
			while($i < $l - 1)
			{
				...
				if (abs($pos3 - (($pos2 + $pos1) / 2)) <= $acceptedCommonStartLengthDifference)
				{
					...
					$start3 = trim($start3); // Most errors came from an leading or trailing espace in a short start
					similar_text($start2, $start3, $percent);
	...
	protected function _formatBatteries($batteries, $questionAndItemSeparator)
	{
		...
		foreach ($batteries as $battery)
		{
			...
			foreach ($battery as $var)
			{
				...
				array_shift($qi);
				$var['question']['item'] = trim(implode($qi));
