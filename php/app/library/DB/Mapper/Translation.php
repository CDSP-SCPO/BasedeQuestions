...
class DB_Mapper_Translation {
	...
	public function getDbTable()
	{
		...
		{
			$this->setDbTable('DB_Table_Translation');
		}
	...
	/**
	 * @param DB_Model_Translation $translation
	 * @return int
	 */
	public function save(DB_Model_Translation $translation)
	{
	...
	protected function _findByIds($ids)
	{
        ...
        for ($i = 0; $i < $l; $i++)
        {
        	$translations[] = $this->_hydrate($results[$i]);
        }
	...
	{
		$translation = new DB_Model_Translation;
		$translation->set_id($row->id);
		return $translation;
	}
	...
	{
		$table = $this->getDbTable();
		$query = 'DELETE FROM translations WHERE id IN (';
		$l = count($ids);
